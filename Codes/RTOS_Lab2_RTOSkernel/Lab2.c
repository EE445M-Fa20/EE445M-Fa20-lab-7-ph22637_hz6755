...
void ButtonWork(void){
  ...
  ST7735_Message(1,3,"Jitter0.1us=",MaxJitter);
  PD1 ^= 0x02;
...
int realmain(void){     // realmain
  ...
  OS_AddSW1Task(&SW1Push,2); // Jitter bug(debouce will call OS_Cleartime, cause huge Jitter),see SW1Push()
  OS_AddPeriodicThread(&DAS,PERIOD,1); // 2 kHz real time sampling of PE3 Timer4A done,no problem

	// create initial foreground threads
  NumCreated = 0;  
	
	NumCreated += OS_AddThread(&PID,128,4);//done, no problem
  NumCreated += OS_AddThread(&Interpreter,128,3); //done, no problem
	NumCreated += OS_AddThread(&Consumer,128,1); //run 20s then stopped
...
void Thread2c(void){
  ...
  for(;;){
    PD1 ^= 0x02;       // heartbeat
    Count2++;
    NumCreated += OS_AddThread(&Thread1c,128,3); 
    OS_Sleep(5);
  }
}
void Thread3c(void){
...
int Testmain3(void){  // Testmain3
  ...
  PortD_Init();       // profile user threads
  NumCreated = 0 ;
  NumCreated += OS_AddThread(&Thread2c,128,1); 
  NumCreated += OS_AddThread(&Thread3c,128,2); 
...
void ThreadCS1(void){       // only thread running
  while(1){
...
int TestmainCS(void){       // TestmainCS
  ...
  NumCreated += OS_AddThread(&ThreadCS,128,0); 
	//NumCreated += OS_AddThread(&ThreadCS1,128,0); 

...
uint32_t BackCreated;  // number of periodic background threads created
...
int Testmain5(void){   // Testmain5
  ...
  OS_AddSW2Task(&BackgroundThread5e,2);
...
void Thread6(void){  // foreground thread
  ...
  for(;;){
    Count1++; 
    PD3 ^= 0x08;        // debugging toggle bit 0  
  }
}
...
void TaskA(void){       // called every {1000, 2990us} in background
  ...
  PseudoWork(workA*counts1us); //  do work (100ns time resolution)
		...
		uint32_t thisTime;
		long jitter;
		if(CountA>1){    // ignore timing of first interrupt
      uint32_t diff = OS_TimeDifference(LastTime,thisTime);
      if(diff>TIME_1MS){
        jitter = (diff-TIME_1MS+4)/8;  // in 0.1 usec
      }else{
        jitter = (TIME_1MS+4)/8;  // in 0.1 usec
      }
      if(jitter > MaxJitter){
        MaxJitter = jitter; // in usec
      }       // jitter should be 0
...
void TaskB(void){       // called every pB in background
  ...
  PseudoWork(workB*counts1us); //  do work (100ns time resolution)
		...
		uint32_t thisTime;
		long jitter;
		if(CountB>1){    // ignore timing of first interrupt
      uint32_t diff = OS_TimeDifference(LastTime,thisTime);
      if(diff>(2*TIME_1MS)){
        jitter = (diff-(2*TIME_1MS)+4)/8;  // in 0.1 usec
      }else{
        jitter = ((2*TIME_1MS)-diff+4)/8;  // in 0.1 usec
      }
...
int Testmain6(void){       // Testmain6 Lab 3
  ...
  OS_AddPeriodicThread(&DAS,TIME_500US,3);           // 1 ms, higher priority
  OS_AddPeriodicThread(&TaskB,TIME_1MS,3);         // 2 ms, lower priority

...
int Error_Type;				//Used for buck error type recording
int Error_Num;				//Used for error voltage recording
int Error_Send;
...
//	SYSCTL_RCGC0_R |= SYSCTL_RCGC0_PWM0;      // activate PWM clock for port
//	SYSCTL_RCGC2_R |= SYSCTL_RCGC2_GPIOD;
  GPIO_PORTD_AFSEL_R |= 0x03;   // enable alt funct on PD0&1
	...
	PWM1_0_CTL_R = 0x000000000;
	PWM1_0_GENA_R |= 0x0000008C;
	PWM1_0_GENB_R |= 0x0000080C;
	PWM1_0_LOAD_R |=  0x0000018F;
	PWM1_0_CMPA_R |=  0x0000012B;
	PWM1_0_CMPB_R |=  0x00000063;
	PWM1_0_CTL_R |= 0x00000001;
	PWM1_ENABLE_R |= 0x00000003;	
}  
...
//------------------Buck Task--------------------------------
// foreground thread, runs to modify output voltage
...
void Buck_Fun(void){
  ...
  while(1) {
		...
		PWM1_ENABLE_R |= 0x00000003;
		PWM1_0_CMPA_R =  400*1024/Error_Num;
		PWM1_0_CMPB_R =  400-PWM1_0_CMPA_R;
...
void FetchWeather(void){ uint32_t len; char *p; char *s; char *e; 
  ...
  if(!ESP8266_MakeTCPConnection("api.openweathermap.org", 80, 0)){ // open socket to web server on port 1300
...
void WebServer(void){
  ...
  ST7735_DrawString(0,1,"Wifi connected",ST7735_GREEN);
  if(!ESP8266_StartServer(1849,600)) {  // port 80, 5min timeout
